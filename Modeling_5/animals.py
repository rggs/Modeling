...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 18 21:04:38 2020
...
class Animal():
    ...
    def act(self):
        if self.InWorld.grid[self.xpos,self.ypos]==2 and self.state==1:
            self.state=2
        elif self.InWorld.grid[self.xpos,self.ypos]==3 and self.state==2:
            self.state=3
        elif self.InWorld.grid[self.xpos,self.ypos]==3 and self.state==1:
                self.state=3        
        
        elif self.InWorld.grid[self.xpos,self.ypos]==0:
            self.state=0
        elif self.InWorld.grid[self.xpos,self.ypos]==5 and self.state==4:
            self.state=5
            
        if self.state==1:
            if self.InWorld.hasNeighbors(self.xpos, self.ypos):
                for j in self.InWorld.Neighbors(self.xpos, self.ypos):
                    if self.InWorld.grid[j[0],j[1]]==1:
                        spot=self.InWorld.findSpot(1)
                        self.birth=1
                        if spot=='Fail':
                            pass
                        else: 
                            return Animal(self.InWorld,1,spot[0],spot[1]) #This may cause errors
            ...
            if self.hunger>=5:
                ...
                if self.InWorld.hasNeighbors(self.xpos, self.ypos):
                    for j in self.InWorld.Neighbors(self.xpos, self.ypos):
                        if self.InWorld.grid[j[0],j[1]]==1 or self.InWorld.grid[j[0],j[1]]==2 :
                            self.hunger=0
                            self.InWorld.place(j[0],j[1],3)
                            return [j[0],j[1]]
            ...
            if self.birth%9==0:
                spot=self.InWorld.findSpot(4)
                self.birth=1
                if spot=='Fail':
                    pass
                else: 
                    return Animal(self.InWorld,4,spot[0],spot[1]) #This may cause errors
            ...
            if self.InWorld.hasNeighbors(self.xpos, self.ypos):
                ...
                if self.InWorld.hasNeighbors(self.xpos, self.ypos):
                    for j in self.InWorld.Neighbors(self.xpos, self.ypos):
                        if self.InWorld.grid[j[0],j[1]]==4:
                            val=np.random.randint(1,11)
                            if val>=7:
                                self.hunger=0
                                self.InWorld.place(j[0],j[1],5)
                                return [j[0],j[1]]
